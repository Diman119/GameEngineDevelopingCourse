...
void GameFramework::Init()
{
	...
	flecs::entity cubeMoving1 = m_World.entity()
		.set(Position{ 20.f, 0.f, 4.f })
		.set(Velocity{ -3.f, 5.f, 0.f })
		.set(SphereCollider{ 1.f })
		.set(Gravity{ 0.f, -9.8065f, 0.f })
		.set(BouncePlane{ 0.f, 1.f, 0.f, 5.f })
		.set(Bounciness{ 1.f })
	...
	for (int i = 20; i > 0; --i) {
		float i1 = static_cast<float>(i);
		m_World.entity()
			.set(Position{ -30.f + i1 * 3.f, 4.f, 12.f })
			.set(DestructionTimer{ i1 / 2.f })
			.set(EntitySystem::ECS::GeometryPtr{ RenderCore::DefaultGeometry::Cube() })
			.set(EntitySystem::ECS::RenderObjectPtr{ new Render::RenderObject() });
	}
	...
	flecs::entity camera = m_World.entity()
		...
		.set(ControllerPtr{ new Core::Controller(Core::g_FileSystem->GetConfigPath("Input_default.ini")) })
		.set(Shooter{ 0.1f, 0.6f, 20.f, 6, 6, 0.f });
}
...
void GameFramework::RegisterComponents()
{
	...
	ECS_META_COMPONENT(m_World, SphereCollider);
